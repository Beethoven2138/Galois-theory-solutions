...
\end{exercise}
\begin{proof}
Let $F$ be the set of elements of $G$ with infinite order and the identity element. $F$ is a subgroup. Let $T$ be the torsion subgroup. We then have $G\cong F\oplus T$. $F\cong\mathbb{Z}^s$ where $s$ is defined as in the previous exercise. Then $G\cong\mathbb{Z}^s\oplus T$.
\end{proof}
